...
def exit_gracefully(
    driver: WebDriver, connection: MySQLConnection, user_data_dir: str, exit_code: int
) -> None:
    driver.quit()
    connection.close()
...
def get_price_from_user(actual_listing: Dict) -> float:
    ...
    while True:
        try:
            max_price = round_up_decimal(
                input(
                    f"Set the maximum buy order at max for this item: {name}, your current buy order price: {price}\n"
                )
            )
            if max_price > price:
                return max_price
...
def get_action_from_user() -> str:
    print(
        "Set the action if the buy order price goes above max. I or i to ignore, C or c to cancel the buy order."
    )
...
def filter_listings_to_knives(listings: Dict) -> List[Dict]:
    ...
    for order_id in orders:
        ...
        if game_name == "Counter-Strike 2" and any(
            ...
            filtered_order = {}
            filtered_order["price"] = (
                actual_listing.get("price")
                .replace(",", ".")
                .replace("-", "0")
                .replace("€", "")
                .replace(" ", "")
...
def delete_knives_from_csv(
    knives_to_delete: List[str] | Set[str], file_path: str
) -> None:
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        ...
        if not reader.fieldnames:
            logger.fatal(
                "Unexpectedly csv file has no headers while deleting, exiting..."
            )
            exit_gracefully(driver, connection, user_data_dir, 1)
        ...
        if item2:
            for key in item1:
                ...
                if value1 != value2:
                    ...
                    logger.info(
                        f"  Column '{key}': '{item1[key]}' (actual) vs '{item2[key]}' (file)"
                    )
        else:
            logger.info(
                f"Order {name} in actual is missing from file. We will add it to the file"
            )
...
def check_for_missing_orders(
    actual: List[Dict], file: List[Dict], file_path: str
) -> None:
    ...
    for element in missing_in_actual:
        logger.info(
            f"INFO: Order {element} is missing, you either already bought it or manually canceled it. We will delete it from csv."
        )
...
def handle_user_choice() -> str:
    print(
        "If you want to cancel the order press C or c, if you want to put a new max price press N or n, if you want to ignore this press I or i"
    )
...
def cancel_order(order: Dict) -> bool:
    try:
        ...
        if order_id is None:
            logger.error(
                f"Failed to cancel the buy order on the knife {order.get('item_name')} because order has no id"
            )
            return False
        response = steam_client.market.cancel_buy_order(int(order_id))
    except Exception as e:
        logger.error(
            f"Failed to cancel the buy order on the knife {order.get('item_name')} skipping..."
        )
        logger.error(str(e))
        return False
    if response.get("success") != 1:
        ...
        logger.info(
            f"Successfully canceled the buy order on the knife {order.get('item_name')} at price {order.get('price')}"
        )
        return True
...
def put_order(listing: Knife, price: int) -> Tuple[bool, int]:
    try:
        response = steam_client.market.create_buy_order(
            listing.knife_name, str(price), 1, GameOptions.CS, Currency.EURO
        )

    except Exception as e:
        logger.error(
            f"Failed to put the buy order on the knife {listing.knife_name}, skipping..."
        )
        logger.error(str(e))
        return False, -1
    if response.get("success") != 1:
        ...
        logger.info(
            f"Successfully put the buy order on the knife {listing.knife_name} at price {price / 100.0}"
        )
...
def amend_orders(orders):
    unsuccessfully_amended = []
    successfully_amended = []
    for order in orders:
...
def get_wallet_balance(steam_client: SteamClient) -> Tuple[bool, float]:
    try:
        ...
        logger.error(
            "Failed to get wallet balance sleeping for 30 seconds then trying again"
        )
        time.sleep(30)
        return False, -1
        ...
        try:
            ...
            for knife_order in knife_orders_file:
                ...
                if knife_name is None:
                    continue
                knife_listing = safe_get_knife_info(
                    (knife_name,), driver, cursor, connection, 6, logger
                )
                ...
                if knife_listing.buy_order_price is None:
                    ...
                    continue
                price_difference = (
                    knife_listing.buy_order_price - current_buy_order_price
                )
                ...
                if (price_difference > 0) and (
                    knife_listing.buy_order_price < knife_max_price
                ):
                    if cancel_order(knife_order):
                        if knife_listing.buy_order_price < wallet_balance:
                            success, id = put_order(
                                knife_listing,
                                int(knife_listing.buy_order_price * 100 + 1),
                            )
                            if success:
                                delete_knives_from_csv([knife_name], file_path)
                                new_order = knife_order
                                new_order["price"] = round_up_decimal(
                                    knife_listing.buy_order_price + 0.01
                                )
                                new_order["order_id"] = id
                                add_knives_to_csv([new_order], file_path)
                        else:
                            success, id = put_order(
                                knife_listing, int(wallet_balance * 100)
                            )
                            if success:
                                delete_knives_from_csv([knife_name], file_path)
                                new_order = knife_order
                    ...
                    if knife_order.get("action") == "cancel" and cancel_order(
                        knife_order
                    ):
                        logger.info(
                            f"Canceled order on knife {knife_name} because price is bigger than maximum set."
                        )
