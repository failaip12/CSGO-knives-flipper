        ...
        for knife_order in knife_orders:
            ...
            if (price_difference > 0) and (current_buy_order_price != wallet_balance):
                ...
                if knife_listing["buy_order_price"] > wallet_balance:
                    print(
                        "INFO: Current max buy order price is higher than the amount of money you have in wallet, setting the price to your wallet money as fallback",
                        knife_name)
                    try:
                        response = steam_client.market.create_buy_order(knife_name, wallet_balance * 100, 1,
                                                                        GameOptions.CS, Currency.EURO)
                    except Exception as e:
                        print(e)
                else:
                    try:
                        ...
                        # print("---------------------------------------------------")
                        response = steam_client.market.create_buy_order(knife_name,
                                                                        knife_listing["buy_order_price"] * 100 + 5, 1,
                                                                        GameOptions.CS, Currency.EURO)
                    except Exception as e:
                        print(e)
                if response.get('success') != 1:
                    ...
                    print("INFO: Succsessfully created the buy order on the knife ", knife_name, ", for the price of ",
                          knife_listing["buy_order_price"] + 0.05)
        time.sleep(150)
