...
def steam_login() -> WebDriver:
    ...
    options = Options()
    options.add_argument('--disable-gpu')
    options.add_argument(f"user-data-dir={original_user_data_dir}")  # Use the copied unique user data dir per thread
    chrome_driver = webdriver.Chrome(options=options)
    chrome_driver.request_interceptor = interceptor  # Attach interceptor here
...
def update_all_knife_data(failed_knives_name: str, logger: CustomLogger, date: Optional[str] = None, wait_time: int = 6) -> None:
    try:
        ...
        handle_shutdown(None, None, logger)  # Trigger the signal handler for a clean shutdown
